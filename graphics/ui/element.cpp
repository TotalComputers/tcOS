...
float PositionedElement::getY() const {
    return y;
}

float PositionedElement::getWidth() const {
    return w;
}

float PositionedElement::getHeight() const {
    return h;
}
...
int CachedElement::getSamples() const {
    return numSamples;
}
...
void CachedElement::cache(int layers) {
    const int prevSize = (int) this->layers.size();
    ...
    for (int i = 0; i < layers; i++) {
        if (numSamples <= 1) {
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, (int) getWidth(), (int) getHeight(), 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
...
void CachedElement::display() {
    ...
    Shader::uniformMat4f((int) matrixLoc, matrix);
